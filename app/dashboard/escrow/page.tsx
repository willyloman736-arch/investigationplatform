...
export const metadata: Metadata = {
  title: `Escrow Account Dashboard · ${APP_NAME}`,
  description:
    "Manage the escrow workspace connected to an existing recovery complaint.",
};
...
export default async function DashboardOverviewPage() {
  ...
  if (!primaryOperation || operations.length === 0) {
    return <EmptyEscrowDashboard firstName={firstName(user.name)} hasCase />;
  }
...
function EscrowPrerequisiteGate({ firstName }: { firstName: string }) {
  return (
    <SecureEscrowAccountCard
      firstName={firstName}
      title="Recovery Case Required"
      body="Open a recovery complaint first. Once your case is on file, your secure escrow workspace becomes available for recovered funds, withdrawal review, receipts, and protected messaging."
      primaryHref="/register"
      primaryLabel="Open Recovery Case"
      secondaryHref="/dashboard/cases"
      secondaryLabel="View Recovery Dashboard"
      locked
    />
  );
}
...
function EmptyEscrowDashboard({
  ...
  return (
    <SecureEscrowAccountCard
      firstName={firstName}
      title="Create Your Secure Escrow Account"
      body={
        hasCase
          ? "Your recovery case is on file. Continue into the secure escrow workspace to monitor escrow status once recovered funds and provider confirmations are available."
          : "A recovery complaint must be opened before escrow access can be enabled."
      }
      primaryHref={hasCase ? "/dashboard/cases" : "/register"}
      primaryLabel={hasCase ? "Continue With Recovery Case" : "Open Recovery Case"}
...
function SecureEscrowAccountCard({
  firstName,
  title,
  body,
  primaryHref,
  ...
  return (
    <section className="mx-auto max-w-[520px] rounded-[2rem] border border-white/15 bg-[#041122]/95 p-4 text-white shadow-[0_28px_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl sm:p-6">
      ...
      <div className="mt-7 text-center">
        <div className="mx-auto flex items-center justify-center gap-2">
          <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-emerald-300/25 bg-emerald-400/10 text-emerald-300">
            {locked ? <Lock className="h-6 w-6" /> : <ShieldCheck className="h-6 w-6" />}
          </span>
          <span className="text-xl font-semibold tracking-tight">
            Secure<span className="text-emerald-300">Escrow</span>
          </span>
        </div>
        <p className="mt-5 text-sm text-slate-400">Good morning, {firstName}</p>
        <h1 className="mt-2 text-[2rem] font-semibold leading-tight tracking-tight sm:text-4xl">
          {title}
        </h1>
      ...
      <div className="mt-6 grid grid-cols-4 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.055]">
        <EscrowTrustItem icon={Lock} label="SSL Secured" />
        <EscrowTrustItem icon={IdCard} label="Identity Review" />
        <EscrowTrustItem icon={MessageSquare} label="Protected Comms" />
        <EscrowTrustItem icon={Wallet} label="Secure Withdrawals" />
      </div>

      <div className="mt-5 rounded-[1.5rem] border border-white/10 bg-white/[0.055] p-4">
        <div className="rounded-2xl border border-white/10 bg-slate-950/30 px-4 py-3">
          ...
          <div className="mt-2 flex items-center gap-3 text-sm text-slate-300">
            ...
            <span>{locked ? "Required before escrow setup" : "Linked case available"}</span>
          </div>
        </div>
        ...
        <Button asChild variant="outline" className="mt-3 h-12 w-full rounded-2xl border-white/10 bg-white/[0.055]">
...
function EscrowTrustItem({
  ...
  return (
    <div className="border-r border-white/10 px-2 py-3 text-center last:border-r-0">
    ...
    <div className="flex items-center gap-2 text-sm text-slate-300">
      <span
        className={cn(
          ...
          complete ? "bg-blue-500 text-white" : "bg-white/[0.08] text-slate-500"
        )}
      >
